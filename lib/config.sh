 ...
 while IFS= read -r raw_line||  [[ -n $raw_line   ]];do
  ...
  if [[ $line =~ ^([a-zA-Z_]+)[[:space:]]*=[[:space:]]*(.*)$   ]];then
   ...
   if [[ $current_section == "DEFAULT"   ]];then
    ...
    path)warn "'path' in [DEFAULT] is not supported (line $line_num)"
     last_key=""
;;
    *)last_key=""
...
get_project_prop()
 ...
 local prop="$2"
 case "$prop" in
 path)echo "${PROJECT_PATH[$project]:-}"
;;
 session_name)local name="${PROJECT_SESSION_NAME[$project]:-${DEFAULTS[session_name]:-}}"
  [[ -z $name   ]]&&  name="$project"
  echo "${name//./-}"
;;
